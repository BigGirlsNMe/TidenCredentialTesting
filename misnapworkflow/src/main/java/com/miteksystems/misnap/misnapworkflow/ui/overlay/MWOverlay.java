/*
 ...
 The MWOverlay class serves to greatly simplify the addition of a dynamic viewfinder to your own application.
...
public class MWOverlay extends View{
	...
	private static final boolean enableSimpleOverlay = true;
	private static boolean hideAllOverlay = false;
	...
	public static void removeOverlay () {
		...
		/*animation = locationLayer.getAnimation();
		if (animation != null){
			animation.cancel();
			animation.reset();
		}*/
    ...
    public static void resetOverlay(){
	 ...
	 public void onDraw(Canvas canvas)
	 {
		...
		/*if (getResources().getConfiguration().orientation == Configuration.ORIENTATION_PORTRAIT){
         ...
         if(!hideAllOverlay){
             if(enableSimpleOverlay){
                 if (layerType == LayerType.LT_VIEWPORT){
                     drawViewPort(canvas, frame);
                 }
             }else{
                 if (layerType == LayerType.LT_LOCATION && location != null && locationLayer.location!= null/*&& locationLayer.getVisibility() == View.VISIBLE*/){
                     drawLocation(canvas);
                 } else if (layerType == LayerType.LT_VIEWPORT){
                     drawViewPort(canvas, frame);
                 } else{
                     drawBlinkngLines(canvas, frame);
                 }
             }
         }
	 }

    private void drawBlinkngLines(Canvas canvas, RectF frame){
    ...
    private void drawViewPort(Canvas canvas, RectF frame){
        Paint paint = new Paint();
        int width = canvas.getWidth();
        int height = canvas.getHeight();
        if (isViewportVisible != (viewportLayer.getVisibility() == View.VISIBLE)){
